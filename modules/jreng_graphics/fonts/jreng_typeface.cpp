 ...
 * | File                      | Responsibility                                    |
 * |---------------------------|---------------------------------------------------|
 * | `jreng_font_metrics.cpp`  | `calcMetrics()` — FreeType cell geometry          |
 * | `jreng_font_shaping.cpp`  | `shapeText()`, `shapeEmoji()`, static utilities   |
 *
 ...
 *    `FT_New_Memory_Face`.
 * 8. Register the regular face and Nerd Font face with `Registry` at
 ...
 * shaping fonts update automatically).  Updates `Registry` slot 1.
 *
 * ### Rasterization
 * Implemented via `rasterizeToImage()`.  Calls
 ...
 * 5. **Registry** — registers the regular face at slot 0, calls
 *    `populateFromCmap (0)`.  Loads the Nerd Font from
 *    `BinaryData::SymbolsNerdFontRegular_ttf` via `FT_New_Memory_Face`, sets
 *    character size, creates `nerdShapingFont`, registers at slot 1, calls
 *    `populateFromCmap (1)`.
 *
 ...
 * 5. Updates `Registry` slot 1 with the new `nfFace` and
 ...
 *
 * @note This overload exists in `jreng_font.cpp` for the case where the FreeType
 *       backend is compiled on macOS (e.g. for testing).  In production the
 *       macOS build uses `jreng_font.mm` exclusively.
 *
 * @param familyName  Font family name to look up.
