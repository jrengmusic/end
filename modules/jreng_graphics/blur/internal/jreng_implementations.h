...
// There are also runtime considerations
    ...
    // https://developer.apple.com/documentation/accelerate/4172615-vimagesepconvolve_argb8888
        ...
        #define JRENG_BLUR_VIMAGE 1
        #define JRENG_BLUR_VIMAGE_MACOS14 1
        #include "../implementations/jreng_vImage_macOS14.h"
...
// Don't use these directly, use jreng::CachedBlur!
namespace jreng::blur
{
    [[maybe_unused]] static inline void singleChannel (juce::Image& img, size_t radius)
    {
#if JRENG_BLUR_VIMAGE
        if (internal::vImageSingleChannelAvailable())
            jreng::blur::vImageSingleChannel (img, radius);
        else
            jreng::stackBlur::ginSingleChannel (img, static_cast<unsigned int> (radius));
#elif defined(JRENG_BLUR_IPP)
        ippVectorSingleChannel (img, radius);
#else
        jreng::blur::juceFloatVectorSingleChannel (img, radius);
#endif
    }

    [[maybe_unused]] static inline void argb ([[maybe_unused]] juce::Image& srcImage, juce::Image& dstImage, size_t radius)
    {
#if JRENG_BLUR_VIMAGE_MACOS14
        if (internal::vImageARGBAvailable())
            jreng::blur::vImageARGB (srcImage, dstImage, radius);
        else
            jreng::stackBlur::ginARGB (dstImage, static_cast<unsigned int> (radius));
#else
