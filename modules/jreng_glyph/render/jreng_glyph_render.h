 ...
 *
 * @note Shaders for these render types live in `modules/jreng_glyph/shaders/`.
 *       They are currently loaded via app-level `BinaryData::getString()`.
 *       A module-level shader loading mechanism is required in a future sprint
 *       before the app's BinaryData dependency can be removed.
 *
 ...
 * coordinates, and foreground colour.  Arrays of `Quad` are uploaded to the
 ...
 *
 * @par Thread contract
 * - **READER THREAD** (MESSAGE THREAD): builds and writes `Quad` instances
 *   into `Render::SnapshotBase`.
 * - **GL THREAD**: reads `Quad` instances from the acquired snapshot
 ...
 * frames.
 *
 * Two instances are owned internally by `jreng::GLSnapshotBuffer` and
...
static_assert (std::is_trivially_copyable_v<Render::Quad>,       "jreng::Glyph::Render::Quad must be trivially copyable for GPU upload");
