 ...
 * FreeType equivalent lives in `jreng_font_registry.cpp`.
 *
 * ## Algorithm
 *
 ...
 * codepoint in the range [0, `unicodeMax`) and tests membership with
 ...
 * `entries[slotIndex]` and iterates all Unicode codepoints [0, `unicodeMax`).
 ...
 *       returning.
 *
 * @note This is an O(unicodeMax) scan (~1 M iterations) and should only be
...
void Font::Registry::populateFromCmap (int slotIndex) noexcept
{
    if (slotIndex >= 0 and slotIndex < entryCount)
    {
        ...
        if (entry.ctFont != nullptr)
        {
            ...
            if (charSet != nullptr)
            {
                for (uint32_t cp { 0 }; cp < static_cast<uint32_t> (unicodeMax); ++cp)
                {
