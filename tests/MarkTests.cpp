 ...
 *
 * Coverage: RFC-vt-correctness.md V3 decision (ghostty 4-state vocabulary),
 * PLAN-vt-correctness.md Step 3. Two independent surfaces:
 ...
 *      populate CellFifo in this PLAN ("Consumers are RFC-terminal-editor
 *      scope" — PLAN Step 3).
 */
...
TEST_CASE ("OSC 133;C stamps output, 133;D closes the block without re-stamping", "[video][osc133][v3]")
{
    ...
    REQUIRE (t.line (1).mark() == jam::CodeLine::Mark::output);
    ...
    REQUIRE (t.line (1).mark() == jam::CodeLine::Mark::output);
    REQUIRE (t.line (2).mark() == jam::CodeLine::Mark::none);
}
...
TEST_CASE ("prompt continues as promptContinuation across a line feed", "[video][osc133][v3][promptcontinuation]")
{
    ...
    REQUIRE (t.line (0).mark() == jam::CodeLine::Mark::prompt);
    REQUIRE (t.line (1).mark() == jam::CodeLine::Mark::promptContinuation);
}
...
TEST_CASE ("CellFifo pushActive/drainActive round-trips isContinued/isJustified/mark", "[cellfifo][transport][v3]")
{
    ...
    const uint8_t flags
    {
        static_cast<uint8_t> (jam::terminal::CellFifo::isContinuedFlag
                             ...
                             | (static_cast<uint8_t> (jam::CodeLine::Mark::input) << jam::terminal::CellFifo::markShift))
    };
    ...
    REQUIRE (line.mark == jam::CodeLine::Mark::input);
    REQUIRE (line.chars[0].codepoint() == uint32_t ('a'));
    REQUIRE (line.chars[2].codepoint() == uint32_t ('c'));
}

TEST_CASE ("CellFifo drainHistory joins isContinued runs into one CodeLine", "[cellfifo][transport][v3]")
{
    jam::terminal::CellFifo fifo (256, 256);
    ...
    const uint8_t markBits { static_cast<uint8_t> (jam::CodeLine::Mark::output) << jam::terminal::CellFifo::markShift };
    ...
    REQUIRE (line.mark == jam::CodeLine::Mark::output);   // mark taken from the FIRST physical row
    REQUIRE (line.chars[0].codepoint() == uint32_t ('a'));
