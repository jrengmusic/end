 ...
 *        stamping on head and spacerTail, `;;` reset, flexGap preserves
...
TEST_CASE ("OSC 8 stamps linkId on both the doubleWidth head and the spacerTail", "[video][osc8][v5][width]")
{
    Test::Term t { 20, 2 };
    t.feed ("\x1b]8;;https://example.com\x07");
...
TEST_CASE ("flexGap merge preserves the merge target's own linkId, not the current pen", "[video][osc8][v5][flexgap]")
{
    ...
    t.feed (" ");            // second space — triggers the flexGap merge

    const auto first  { t.cell (0, 0) };
    const auto second { t.cell (0, 1) };

    REQUIRE (t.line (0).isJustified());
    REQUIRE (first.contentTag()  == jam::Char::flexGap);
    REQUIRE (second.contentTag() == jam::Char::flexGap);
