 ...
 *
 * @param daemonRef  Owning Daemon — used in connectionLost() to release this object
 *                   and for all subscriber/broadcast registry operations.
 * @param nexusRef   Session pool — used in messageReceived for session lookup and creation.
 */
Daemon::Channel::Channel (Daemon& daemonRef, Nexus& nexusRef)
 ...
 */
void Daemon::Channel::connectionMade()
{
    daemon.attach (*this);
 ...
 * @note NEXUS PROCESS MESSAGE THREAD.
 */
void Daemon::Channel::connectionLost()
{
    daemon.detach (*this);
 ...
 * @note NEXUS PROCESS MESSAGE THREAD.
 */
void Daemon::Channel::sendPdu (Message kind, const juce::MemoryBlock& payload)
{
 ...
 * @note NEXUS PROCESS MESSAGE THREAD.
 */
void Daemon::Channel::messageReceived (const juce::MemoryBlock& message)
{
...
Daemon::Channel::SpawnPayload
Daemon::Channel::parseSpawnPayload (const uint8_t* payload, int payloadSize)
{
    SpawnPayload out;
    int cursor { 0 };
