...
{
public:
    /** @brief Message-thread IPC connection representing one connected client. */
     ...
     * @note NEXUS PROCESS MESSAGE THREAD.
     */
    void attachSession (const juce::String& uuid, Channel& target, bool sendHistory, int cols, int rows);
     ...
     *
     * Called when a "SESSION" child is appended to `nexus.events`.  Wires
     * daemon-mode IPC callbacks on the newly created session.
     *
     * @note NEXUS PROCESS MESSAGE THREAD.
     */
