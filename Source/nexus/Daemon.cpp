...
Daemon::Daemon (Nexus& host)
    : nexus (host)
{
    nexus.events.addListener (this);
}

/**
 * @brief Stops the daemon and unregisters from Nexus events.
 */
Daemon::~Daemon()
{
    nexus.events.removeListener (this);
    stop();
}
 ...
 *
 * When a "SESSION" child is appended to `nexus.events`, wires daemon-mode IPC
 * callbacks on the newly created session via `wireSessionCallbacks`.
 *
 * @note NEXUS PROCESS MESSAGE THREAD.
 */
void Daemon::valueTreeChildAdded (juce::ValueTree&, juce::ValueTree& child)
{
