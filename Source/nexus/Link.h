...
{
public:
    /** @brief Magic header — single source of truth is nexus::wireMagicHeader in EncoderDecoder.h. */
...
private:
    ...
    // juce::ValueTree::Listener — Nexus::events session lifecycle callbacks.
     ...
     *
     * Populated in valueTreeChildAdded when a new client-mode session is created.
     * Used in valueTreePropertyChanged to match an incoming PARAM tree against a known
     * session root when forwarding cols/visibleRows changes to the daemon via sendResize.
     * Entries are inserted in valueTreeChildAdded and erased in valueTreeChildRemoved.
     *
