    ...
    if (SUCCEEDED (hr) and factory != nullptr)
    {
        ...
        if (stream != nullptr)
        {
            ...
            if (SUCCEEDED (hr) and decoder != nullptr)
            {
                ...
                if (nativeFrameCount > 1u and isGif)
                {
                    ...
                    if (meta.isValid and not meta.frames.empty())
                    {
                        ...
                        {
                            const GifFrameInfo& prev { meta.frames[static_cast<size_t> (
                                i > 0 ? i - 1 : 0)] };
                            ...
                            if (i > 0)
                            {
                                if (prev.disposal == 2)
                                {
                                    ...
                                    const int clearRight  { juce::jlimit (0, canvasW,
                                                                          prev.left + prev.width) };
                                    const int clearBottom { juce::jlimit (0, canvasH,
                                    ...
                                    for (int row { clearTop }; row < clearBottom; ++row)
                                    {
                                        uint8_t* dst { canvas.get()
                                                       + static_cast<size_t> (
                                                           row * canvasW + clearLeft) * 4u };
                                        std::memset (dst, 0,
                                                     static_cast<size_t> (
                                                         clearRight - clearLeft) * 4u);
                                    }
                                }
                                    ...
                                    if (SUCCEEDED (hr) and converter != nullptr)
                                    {
                                        ...
                                        if (SUCCEEDED (hr))
                                        {
                                            ...
                                            juce::Image subImg (juce::Image::ARGB,
                                                                ...
                                                                true);
                                            juce::Image::BitmapData bm (
                                                subImg, juce::Image::BitmapData::writeOnly);
                                            ...
                                            const int blitLeft   { juce::jlimit (0, canvasW,
                                                                                  fi.left) };
                                            const int blitTop    { juce::jlimit (0, canvasH,
                                                                                  fi.top) };
                                            const int blitRight  { juce::jlimit (0, canvasW,
                                                                                  fi.left + iSubW) };
                                            const int blitBottom { juce::jlimit (0, canvasH,
                                                                                  fi.top + iSubH) };
                ...
                else if (nativeFrameCount > 1u)
                {
                    ...
                    hr = decoder->GetFrame (0u, &firstFrame);

                    if (SUCCEEDED (hr) and firstFrame != nullptr)
                    {
                        UINT canvasWu { 0u };
                        UINT canvasHu { 0u };
                        firstFrame->GetSize (&canvasWu, &canvasHu);
                        firstFrame->Release();

                        const int canvasW { static_cast<int> (canvasWu) };
                        const int canvasH { static_cast<int> (canvasHu) };

                        if (canvasW > 0 and canvasH > 0)
                        {
                            const size_t frameBytes { static_cast<size_t> (canvasW)
                                                      * static_cast<size_t> (canvasH) * 4u };
                            const size_t totalBytes { frameBytes
                                ...
                                if (SUCCEEDED (hr) and wicFrame != nullptr)
                                {
                                    ...
                                    if (fW > 0u and fH > 0u)
                                    {
                                        ...
                                        if (SUCCEEDED (hr) and converter != nullptr)
                                        {
                                            ...
                                            if (SUCCEEDED (hr))
                                            {
                                                juce::Image frameImg (juce::Image::ARGB,
                                                                      static_cast<int> (fW),
                                                                      static_cast<int> (fH),
                                                                      true);
                                                juce::Image::BitmapData bm (
                                                ...
                                                converter->CopyPixels (
                                                    nullptr,
                                                    static_cast<UINT> (bm.lineStride),
                                                    static_cast<UINT> (bm.lineStride) * fH,
                                                ...
                                                std::memcpy (
                                                    seq.pixels.get()
                                                        + static_cast<size_t> (i) * frameBytes,
                                                    frameRgba.get(),
                                                    frameBytes);
                                            }
