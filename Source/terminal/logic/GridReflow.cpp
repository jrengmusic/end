 ...
 * @param srcOffset     Offset (in cells) into `src` / `srcG` for this output row.
 ...
 */
static void writeNewRow (Cell* dstCells, Grapheme* dstGraphemes,
                         RowState* dstStates,
...
static int reflowPass (const WalkParams& wp, Cell* tempCells, Grapheme* tempGraphs,
                       ...
                       Cell* dstCells, Grapheme* dstGraphemes,
                       RowState* dstStates,
                       int newTotalRows, int rowsToSkip) noexcept
{
    *outCursorOutputRow = -1;
    ...
    while (r < wp.linearRows)
    {
        ...
        const int flatLen { flattenLogicalLine (wp.cells, wp.graphemes,
                   ...
                   Buffer& newBuffer, int newCols, int newVisibleRows)
{
    ...
    if (linearRows > 0)
    {
        ...
        const int totalOutputRows { reflowPass (wp, tempCells.get(), tempGraphs.get(),
                                                newCols,
        ...
        reflowPass (wp, tempCells.get(), tempGraphs.get(),
                    newCols,
                    cursorLinear, cursorCol,
                    &cursorOutputRow, &cursorNewCol,
                    newBuffer.cells.get(), newBuffer.graphemes.get(),
