...
struct AtlasPacker
{
    ...
    juce::Rectangle<int> allocate (int width, int height) noexcept
    {
        for (int i { 0 }; i < shelfCount; ++i)
        {
            ...
            if (height <= shelf.height and shelf.currentX + width <= atlasWidth)
            {
                juce::Rectangle<int> region { shelf.currentX, shelf.y, width, height };
        ...
        return juce::Rectangle<int> { 0, shelfY, width, height };
    }
