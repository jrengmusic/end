/**
 * @file FontCollection.cpp
 * @brief FontCollection implementation — lifecycle, lookup, and FreeType cmap population.
 *
 ...
 * `FontCollection.mm` and uses CoreText instead of FreeType.
 *
 * @see FontCollection.h
 * @see FontCollection.mm
 */
 ...
 *
 * Calls `calloc` to zero-allocate `UNICODE_MAX` bytes, then fills the block
...
{
    lookupTable.calloc (static_cast<size_t> (UNICODE_MAX));
    memset (lookupTable.get(), unresolved, static_cast<size_t> (UNICODE_MAX));
}
 ...
 * stored in `entries` are owned by `Fonts` and are not released here.
 */
FontCollection::~FontCollection()
{
}
 ...
 * any codepoint ≥ `UNICODE_MAX` (i.e. out of the valid Unicode range).
 *
 * @param codepoint  Unicode scalar value (0 … 0x10FFFF).
 * @return           Slot index (0–31), `unresolved` (-1), or `notFound` (-2).
 */
int8_t FontCollection::resolve (uint32_t codepoint) const noexcept
{
    int8_t result { notFound };

    if (codepoint < static_cast<uint32_t> (UNICODE_MAX))
    {
 ...
 *               must remain valid for the lifetime of this `FontCollection`.
 */
void FontCollection::addFont (Entry entry) noexcept
{
 ...
 */
FontCollection::Entry* FontCollection::getEntry (int slotIndex) noexcept
{
 ...
 */
const FontCollection::Entry* FontCollection::getEntry (int slotIndex) const noexcept
{
...
#if JUCE_MAC
// CoreText implementation in FontCollection.mm
#else
 ...
 *
 * @see FontCollection.mm  — CoreText equivalent for macOS.
 */
void FontCollection::populateFromCmap (int slotIndex) noexcept
{
    if (slotIndex >= 0 and slotIndex < entryCount)
    {
        ...
        if (entry.ftFace != nullptr)
        {
            ...
            while (glyphIndex != 0)
            {
                if (charcode < static_cast<FT_ULong> (UNICODE_MAX))
                {
