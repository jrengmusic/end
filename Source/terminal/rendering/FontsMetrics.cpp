 ...
 * `Fonts::calcMetrics()`, which derives the logical and physical cell
 ...
 * ### Coordinate spaces
 *
 * `calcMetrics()` populates three coordinate spaces in `Fonts::Metrics`:
 *
    ...
    for (int code { 32 }; code <= 127; ++code)
    {
        ...
        if (glyphIndex != 0)
        {
            ...
            if (loadError == 0)
            {
                ...
                const int cellW { static_cast<int> (ceilf (static_cast<float> (horiAdvance26_6) / static_cast<float> (Fonts::ftFixedScale))) };
 ...
 * @code
 * Metrics m = fonts.calcMetrics (16.0f);
...
Fonts::Metrics Fonts::calcMetrics (float heightPx) noexcept
{
    ...
    if (face != nullptr)
    {
        ...
        if (logicalSizeError == 0)
        {
            ...
            const auto [maxCellW, maxW26_6] { measureMaxCellWidth (face) };
