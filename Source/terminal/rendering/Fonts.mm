/**
 * @file Fonts.mm
 * @brief macOS CoreText + HarfBuzz implementation of the Fonts font manager.
 *
 * This file is compiled **only on macOS** (`JUCE_MAC`).  It implements the
 * `Fonts` struct declared in `Fonts.h` using:
 *
 ...
 * clears `fallbackFontCache`, and updates the live `FontCollection` entries.
 *
...
Fonts::~Fonts()
{
    for (auto& pair : fallbackFontCache)
    {
        ...
        {
            CFRelease (toCTFont (pair.second));
        }
    }
    ...
    {
        CFRelease (toCTFont (identityFont));
    }
    ...
    {
        CFRelease (toCTFont (emojiFont));
    }

    if (mainFont != nullptr)
    {
        CFRelease (toCTFont (mainFont));
    }
}
 ...
 *    `identityShapingFont`.  Registers with `FontCollection` at slot 0 and
 ...
 *    `nerdShapingFont`.  Registers with `FontCollection` at slot 1 and calls
 *    `populateFromCmap (1)`.
 *
 ...
 * This stub satisfies the shared `Fonts` interface.
 *
 * @param  (unused) Style variant.
 * @return Always `nullptr` on macOS.
 */
FT_Face Fonts::getFace (Style) noexcept
{
    return nullptr;
}
 ...
 */
void* Fonts::getFontHandle (Style) noexcept
{
    return mainFont;
}
 ...
 */
hb_font_t* Fonts::getHbFont (Style) noexcept
{
    return shapingFont;
}
 ...
 *         `mainFont` is null.
 */
float Fonts::getPixelsPerEm (Style) noexcept
{
    float result { 0.0f };

    if (mainFont != nullptr)
    {
        result = static_cast<float> (CTFontGetSize (toCTFont (mainFont)));
    }
 ...
 * 7. Updates `FontCollection` slots 0 (`identityFont`) and 1 (`nerdFont`) so
...
void Fonts::setSize (float pointSize) noexcept
{
    ...
    if (mainFont != nullptr)
    {
        auto newFont { CTFontCreateCopyWithAttributes (toCTFont (mainFont),
                                                       scaledSize,
                                                       nullptr,
                                                       nullptr) };
        CFRelease (toCTFont (mainFont));
    ...
    if (emojiFont != nullptr)
    {
        auto newEmojiFont { CTFontCreateCopyWithAttributes (toCTFont (emojiFont),
                                                            scaledSize,
                                                            nullptr,
                                                            nullptr) };
        CFRelease (toCTFont (emojiFont));
    ...
    if (identityFont != nullptr)
    {
        auto newIdentityFont { CTFontCreateCopyWithAttributes (toCTFont (identityFont),
                                                               scaledSize,
                                                               nullptr,
                                                               nullptr) };
        CFRelease (toCTFont (identityFont));
    ...
    if (nerdFont != nullptr)
    {
        auto newNerdFont { CTFontCreateCopyWithAttributes (toCTFont (nerdFont),
                                                           scaledSize,
                                                           nullptr,
                                                           nullptr) };
        CFRelease (toCTFont (nerdFont));
 ...
 */
Fonts::Metrics Fonts::calcMetrics (float heightPx) noexcept
{
    Metrics metrics;

    if (mainFont != nullptr)
    {
        auto scaledFont { CTFontCreateCopyWithAttributes (toCTFont (mainFont),
        ...
        if (scaledFont != nullptr)
        {
            ...
            CGGlyph spaceGlyph { glyphForCodepoint (scaledFont, static_cast<uint32_t> (' ')) };
            float cellW { 0.0f };
 ...
 */
juce::Image Fonts::rasterizeToImage (uint32_t codepoint, float heightPx, bool& isColor) noexcept
{
    juce::Image result;
    isColor = false;
    ...
    if (mainFont != nullptr)
    {
        ...
        if (scaledFont != nullptr)
        {
            CGGlyph cgGlyph { glyphForCodepoint (scaledFont, codepoint) };
            bool useEmojiFont { false };

            if (cgGlyph == 0 and emojiFont != nullptr)
            {
                auto scaledEmoji { CTFontCreateCopyWithAttributes (toCTFont (emojiFont),
                                                                    physicalSize,
                                                                    nullptr,
                                                                    nullptr) };
                ...
                {
                    const CGGlyph emojiGlyph { glyphForCodepoint (scaledEmoji, codepoint) };
            ...
            {
                auto scaledNF { CTFontCreateCopyWithAttributes (toCTFont (nerdFont),
                                                                 physicalSize,
                                                                 nullptr,
                                                                 nullptr) };
                ...
                if (scaledNF != nullptr)
                {
                    const CGGlyph nfGlyph { glyphForCodepoint (scaledNF, codepoint) };
 ...
 *         is not in `mainFont`.
 */
Fonts::ShapeResult Fonts::shapeASCII (uint32_t codepoint) noexcept
{
    ShapeResult result;

    if (mainFont != nullptr)
    {
        CGGlyph cgGlyph { glyphForCodepoint (toCTFont (mainFont), codepoint) };
        ...
        if (cgGlyph != 0)
        {
            ...
            CGSize advance {};
            CTFontGetAdvancesForGlyphs (toCTFont (mainFont), kCTFontOrientationHorizontal, &cgGlyph, &advance, 1);
 ...
 */
Fonts::ShapeResult Fonts::shapeHarfBuzz (Style style,
 ...
 */
Fonts::ShapeResult Fonts::shapeFallback (const uint32_t* codepoints, size_t count) noexcept
{
    ShapeResult result;
    ...
    if (mainFont != nullptr)
    {
        ...
        for (size_t i { 0 }; i < count; ++i)
        {
            ...
            CGGlyph cgGlyph { glyphForCodepoint (toCTFont (mainFont), cp) };
            CTFontRef useFont { toCTFont (mainFont) };
            ...
            if (cgGlyph == 0)
            {
                ...
                if (cacheIt != fallbackFontCache.end())
                {
                    ...
                    {
                        CTFontRef cached { toCTFont (cacheIt->second) };
                        const CGGlyph cachedGlyph { glyphForCodepoint (cached, cp) };
                ...
                else
                {
                    ...
                    if (cfStr != nullptr)
                    {
                        CTFontRef candidate { CTFontCreateForString (toCTFont (mainFont),
                                                                     cfStr,
                        ...
                        if (candidate != nullptr)
                        {
                            ...
                            if (not isLastResort)
                            {
                                const CGGlyph candidateGlyph { glyphForCodepoint (candidate, cp) };
 ...
 * @return ShapeResult; `count == 0` if no glyphs could be shaped.
 *
 * @note The returned pointer into `shapingBuffer` is invalidated by the next
 *       call to any `shape*` method.
 */
Fonts::ShapeResult Fonts::shapeText (Style style,
 ...
 * @param codepoints  UTF-32 codepoint array; must not be null.
 * @param count       Number of codepoints; must be > 0.
 * @return ShapeResult; `count == 0` if `emojiShapingFont` is null or shaping
 *         produced no glyphs.
 *
 * @note The returned pointer into `shapingBuffer` is invalidated by the next
 *       call to any `shape*` method.
 */
Fonts::ShapeResult Fonts::shapeEmoji (const uint32_t* codepoints, size_t count) noexcept
{
    ShapeResult result;
 ...
 */
int Fonts::ceil26_6ToPx (int v26_6) noexcept
{
 ...
 */
int Fonts::roundFloatPxTo26_6 (float px) noexcept
{
 ...
 */
float Fonts::getDisplayScale() noexcept
{
