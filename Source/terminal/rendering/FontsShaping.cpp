 ...
 * shaping methods declared in `Fonts.h` and the fixed-point / display-scale
 * utilities shared with `FontsMetrics.cpp`.
 *
 * ### Shaping pipeline
 *
 ...
 * they are used by both `FontsShaping.cpp` and `FontsMetrics.cpp`.
 *
 * @note All methods run on the **MESSAGE THREAD**.
 *
 * @see Fonts.h
 * @see FontsMetrics.cpp
 * @see FontCollection
 */
 ...
 *       and caches results in `fallbackFontCache`.  See `Fonts.mm` for that path.
 *
