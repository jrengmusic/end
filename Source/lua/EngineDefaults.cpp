...
static void writeDisplayFontDefaults (juce::String& content, const Engine::Display& display)
{
    ...
    content = jam::String::replaceholder (content, "cursor_blink_interval", juce::String (display.cursor.blinkInterval));
...
static void writeDisplayColoursDefaults (juce::String& content, const Engine::Display& display)
{
    ...
    content = jam::String::replaceholder (content, "colours_selection_cursor", colourToHex (display.colours.selectionCursor));

    static const std::array<const char*, 16> ansiKeys {{
        "colours_black",    "colours_red",           "colours_green",        "colours_yellow",
        "colours_blue",     "colours_magenta",       "colours_cyan",         "colours_white",
        "colours_bright_black",   "colours_bright_red",   "colours_bright_green", "colours_bright_yellow",
        "colours_bright_blue",    "colours_bright_magenta", "colours_bright_cyan",  "colours_bright_white"
    }};
    ...
    content = jam::String::replaceholder (content, "colours_status_bar_label_bg", colourToHex (display.colours.statusBarLabelBg));
    content = jam::String::replaceholder (content, "colours_status_bar_label_fg", colourToHex (display.colours.statusBarLabelFg));
    content = jam::String::replaceholder (content, "colours_status_bar_spinner",  colourToHex (display.colours.statusBarSpinner));
...
static void writeDisplayWindowDefaults (juce::String& content, const Engine::Display& display)
{
    ...
    content = jam::String::replaceholder (content, "window_always_on_top",       display.window.alwaysOnTop       ? "true" : "false");
    ...
    content = jam::String::replaceholder (content, "window_confirmation_on_exit", display.window.confirmationOnExit ? "true" : "false");
}
...
static void writeDisplayMiscDefaults (juce::String& content, const Engine::Display& display)
{
    ...
    content = jam::String::replaceholder (content, "action_list_close_on_run",     display.actionList.closeOnRun ? "true" : "false");
    ...
    content = jam::String::replaceholder (content, "action_list_name_font_size",   juce::String (display.actionList.nameSize));
    content = jam::String::replaceholder (content, "action_list_shortcut_font_family", display.actionList.shortcutFamily);
    content = jam::String::replaceholder (content, "action_list_shortcut_font_style",  display.actionList.shortcutStyle);
    content = jam::String::replaceholder (content, "action_list_shortcut_font_size",   juce::String (display.actionList.shortcutSize));
    content = jam::String::replaceholder (content, "action_list_padding_top",    juce::String (display.actionList.paddingTop));
    content = jam::String::replaceholder (content, "action_list_padding_right",  juce::String (display.actionList.paddingRight));
    content = jam::String::replaceholder (content, "action_list_padding_bottom", juce::String (display.actionList.paddingBottom));
    content = jam::String::replaceholder (content, "action_list_padding_left",   juce::String (display.actionList.paddingLeft));
    content = jam::String::replaceholder (content, "action_list_name_colour",      colourToHex (display.actionList.nameColour));
    content = jam::String::replaceholder (content, "action_list_shortcut_colour",  colourToHex (display.actionList.shortcutColour));
    ...
    content = jam::String::replaceholder (content, "action_list_highlight_colour", colourToHex (display.actionList.highlightColour));
...
void Engine::initDefaults()
{
    ...
    // Pane colours
    display.pane.barColour    = juce::Colour (0xff1b2a31); // dark
...
void Engine::writeNexusDefaults (const juce::File& configDir)
{
    ...
    if (not file.existsAsFile())
    {
        ...
        content = jam::String::replaceholder (content, "terminal_scrollback_lines",  juce::String (nexus.terminal.scrollbackLines));
        content = jam::String::replaceholder (content, "terminal_scroll_step",       juce::String (nexus.terminal.scrollStep));
        content = jam::String::replaceholder (content, "terminal_padding_top",       juce::String (nexus.terminal.paddingTop));
        content = jam::String::replaceholder (content, "terminal_padding_right",     juce::String (nexus.terminal.paddingRight));
        content = jam::String::replaceholder (content, "terminal_padding_bottom",    juce::String (nexus.terminal.paddingBottom));
        content = jam::String::replaceholder (content, "terminal_padding_left",      juce::String (nexus.terminal.paddingLeft));
        content = jam::String::replaceholder (content, "terminal_drop_multifiles",   nexus.terminal.dropMultifiles);
        content = jam::String::replaceholder (content, "terminal_drop_quoted",       nexus.terminal.dropQuoted ? "true" : "false");
...
void Engine::writeWhelmedDefaults (const juce::File& configDir)
{
    ...
    if (not file.existsAsFile())
    {
        ...
        // Code blocks
        content = jam::String::replaceholder (content, "code_fence_background", colourToWhelmedHex (whelmed.codeFenceBackground));
        ...
        content = jam::String::replaceholder (content, "token_identifier",   colourToWhelmedHex (whelmed.tokenIdentifier));
        ...
        content = jam::String::replaceholder (content, "token_punctuation",  colourToWhelmedHex (whelmed.tokenPunctuation));
        content = jam::String::replaceholder (content, "token_preprocessor", colourToWhelmedHex (whelmed.tokenPreprocessor));

        // Table
        content = jam::String::replaceholder (content, "table_background",        colourToWhelmedHex (whelmed.tableBackground));
        content = jam::String::replaceholder (content, "table_header_background", colourToWhelmedHex (whelmed.tableHeaderBackground));
        content = jam::String::replaceholder (content, "table_row_alt",           colourToWhelmedHex (whelmed.tableRowAlt));
        content = jam::String::replaceholder (content, "table_border_colour",     colourToWhelmedHex (whelmed.tableBorderColour));
        content = jam::String::replaceholder (content, "table_header_text",       colourToWhelmedHex (whelmed.tableHeaderText));
        content = jam::String::replaceholder (content, "table_cell_text",         colourToWhelmedHex (whelmed.tableCellText));

        // Progress bar
        content = jam::String::replaceholder (content, "progress_background",     colourToWhelmedHex (whelmed.progressBackground));
        content = jam::String::replaceholder (content, "progress_foreground",     colourToWhelmedHex (whelmed.progressForeground));
        content = jam::String::replaceholder (content, "progress_text_colour",    colourToWhelmedHex (whelmed.progressTextColour));
        content = jam::String::replaceholder (content, "progress_spinner_colour", colourToWhelmedHex (whelmed.progressSpinnerColour));
        ...
        content = jam::String::replaceholder (content, "scrollbar_background", colourToWhelmedHex (whelmed.scrollbarBackground));
        content = jam::String::replaceholder (content, "selection_colour",     colourToWhelmedHex (whelmed.selectionColour));
