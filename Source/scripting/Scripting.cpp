 ...
 *
 * Contains: file helpers, Engine constructor/destructor,
...
{
    return juce::File::getSpecialLocation (juce::File::userHomeDirectory).getChildFile (".config/end/action.lua");
}
...
//==============================================================================
Engine::~Engine() { watcher.removeListener (this); }
...
void Engine::load()
{
    ...
    lua = jam::lua::state {};
    lua.open_libraries (jam::lua::lib::base, jam::lua::lib::string, jam::lua::lib::table, jam::lua::lib::math);
    ...
    displayTable.set_function ("focus_pane",
                               [this] (int dx, int dy)
                               {
                                   displayCallbacks.focusPane (dx, dy);
                               });
    ...
    if (actionFile.existsAsFile())
    {
        auto result { lua.safe_script_file (
            actionFile.getFullPathName().toStdString(), jam::lua::script_pass_on_error) };
...
void Engine::registerActions (Action::Registry& registry)
{
    ...
    {
        auto launchFn { [this, popup]() -> bool
                        {
                            popupCallbacks.launchPopup (
                                popup.name, popup.command, popup.args, popup.cwd, popup.cols, popup.rows);
                            return true;
                        } };

        if (popup.modal.isNotEmpty())
        {
            registry.registerAction ("popup:" + popup.name,
                                     "Popup: " + popup.name,
        ...
        {
            registry.registerAction ("popup_global:" + popup.name,
                                     "Popup: " + popup.name,
                                     "Open " + popup.name + " popup",
                                     "Popups",
                                     false,
    ...
    for (auto& action : customActions)
    {
        ...
        registry.registerAction (action.id, action.name, action.description, "Custom", action.isModal, executeFn);
    }
}
...
//==============================================================================
const Engine::SelectionKeys& Engine::getSelectionKeys() const noexcept { return selectionKeys; }

const juce::String& Engine::getLoadError() const noexcept { return loadError; }

//==============================================================================
const juce::String& Engine::getPrefixString() const noexcept { return prefixString; }
